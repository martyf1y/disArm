...
//--------------------------------------------------------------
void ofxLeapMotionSimpleHand::debugDraw(ofxAssimpModelLoader _model, ofColor objectColor){
	ofPushStyle();
...
ofPoint ofxLeapMotion::getMappedofPoint(Vector v){
	...
	p.y = yOffsetOut + (p.y*2 - yOffsetIn) * (yScale);
